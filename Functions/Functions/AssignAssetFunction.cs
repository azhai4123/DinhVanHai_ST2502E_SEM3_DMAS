...
namespace Functions.Functions
{
    public class AssignAssetFunction
    {
        ...
        public async Task<HttpResponseData> Run([HttpTrigger(AuthorizationLevel.Function, "post", Route = "assignasset")] HttpRequestData req)
        {
            ...
            var pa = new PlayerAsset { PlayerAssetId = Guid.NewGuid(), PlayerId = r.PlayerId, AssetId = r.AssetId, Quantity = r.Quantity, AcquiredDate = DateTime.UtcNow };
            _db.PlayerAssets.Add(pa);
            await _db.SaveChangesAsync();

            var res = req.CreateResponse(HttpStatusCode.Created);
            await res.WriteStringAsync(JsonSerializer.Serialize(new { playerAssetId = pa.PlayerAssetId }));
            return res;
        }
    }
}
